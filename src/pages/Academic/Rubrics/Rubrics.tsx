...
import { RubricCriterionDraft, RubricDraft, createInitialDraft } from './hooks/useRubricDraft';
...
const Rubrics = () => {
  ...
  const totalWeight = useMemo(
    () => draft.criteria.reduce((sum: number, criterion: RubricCriterionDraft) => sum + Number(criterion.weight || 0), 0),
    [draft.criteria]
  );
  ...
  const updateField = <K extends keyof RubricDraft>(field: K, value: RubricDraft[K]) => {
    setDraft((current: RubricDraft) => ({ ...current, [field]: value }));
  };
  ...
  const updateCriterion = <K extends keyof RubricCriterionDraft>(
    ...
    setDraft((current: RubricDraft) => ({
      ...current,
      criteria: current.criteria.map((criterion: RubricCriterionDraft) =>
        criterion.id === id ? { ...criterion, [field]: value } : criterion
      ),
    }));
  };

  const updateScale = <K extends keyof import('./hooks/useRubricDraft').RubricScaleDraft>(
    criterionId: string,
    scaleId: string,
    field: K,
    value: import('./hooks/useRubricDraft').RubricScaleDraft[K]
  ) => {
    setDraft((current: RubricDraft) => ({
      ...current,
      criteria: current.criteria.map((criterion: RubricCriterionDraft) =>
  ...
  const addCriterion = () => {
    setDraft((current: RubricDraft) => ({
      ...current,
      criteria: [...current.criteria, createInitialDraft().criteria[0]],
    }));
  };
  ...
  const removeCriterion = (id: string) => {
    setDraft((current: RubricDraft) => ({
      ...
      criteria: current.criteria.length > 1
        ? current.criteria.filter((criterion: RubricCriterionDraft) => criterion.id !== id)
        : current.criteria,
    }));
  };
